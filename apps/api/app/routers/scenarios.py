...
@router.get("/scenarios/{scenario_id}/monitor")
async def get_monitor_data(
    scenario_id: str,
    db: Session = Depends(get_db),
):
    """Performance monitor: capex actuals vs plan, IRR reforecast, milestones."""
    scenario = db.query(Scenario).filter(Scenario.id == scenario_id).first()
    if not scenario:
        raise HTTPException(status_code=404, detail="Scenario not found")

    parsed = _get_model_data(scenario.model_id, db)
    capex_data = parsed.get("capex", {})
    returns_data = parsed.get("returns", {})
    assumptions = parsed.get("assumptions", [])
    cover = parsed.get("cover", {})
    sensitivity_data = parsed.get("sensitivity", {})

    assumption_map: dict[str, Any] = {a["name"]: a["value"] for a in assumptions}
    years = capex_data.get("years", [])
    n_years = len(years)

    # ── Core parameters ──
    total_capex_base = float(assumption_map.get("Total base capex", 850))
    contingency_pct = float(assumption_map.get("Capex contingency %", 0.08))
    total_capex_incl = total_capex_base * (1 + contingency_pct)
    construction_period = int(assumption_map.get("Construction period", 3))
    ops_start_year = int(assumption_map.get("Operations start year", 2028))
    first_year = int(years[0]) if years else 2025
    construction_end_year = first_year + construction_period - 1

    # KPIs for the selected scenario
    scenario_key = _get_scenario_key(scenario)
    metrics = returns_data.get("metrics", [])
    metric_map = {m["metric"]: m for m in metrics}
    base_irr = float(metric_map.get("Project IRR (unlevered)", {}).get(scenario_key, metric_map.get("Project IRR (unlevered)", {}).get("base_case", 0.123)))
    if base_irr > 1:
        base_irr = base_irr / 100
    base_npv = float(metric_map.get("NPV @ WACC (USD M)", {}).get(scenario_key, metric_map.get("NPV @ WACC (USD M)", {}).get("base_case", 145)))
    base_payback = float(metric_map.get("Payback period (years)", {}).get(scenario_key, metric_map.get("Payback period (years)", {}).get("base_case", 9.2)))
    base_dscr_avg = float(metric_map.get("DSCR — average", {}).get(scenario_key, metric_map.get("DSCR — average", {}).get("base_case", 1.45)))
    base_dscr_min = float(metric_map.get("DSCR — minimum (year 2030)", {}).get(scenario_key, metric_map.get("DSCR — minimum (year 2030)", {}).get("base_case", 1.12)))

    # ── Construction capex breakdown by package ──
    cap_d = capex_data.get("data", {})
    construction_total = cap_d.get("Construction capex total", [0] * n_years)

    # Individual packages for milestones
    tank1 = cap_d.get("Tank 1 — full containment (200k m³)", [0] * n_years)
    tank2 = cap_d.get("Tank 2 — full containment (200k m³)", [0] * n_years)
    jetty = cap_d.get("Loading arm & jetty works", [0] * n_years)
    regas = cap_d.get("Regasification equipment", [0] * n_years)
    pipeline = cap_d.get("Sub-sea pipeline (Phase 2)", [0] * n_years)
    utilities = cap_d.get("Utilities & OSBL", [0] * n_years)
    inst_ctrl = cap_d.get("Instrumentation & control systems", [0] * n_years)
    epcm = cap_d.get("EPCM / Owner costs", [0] * n_years)
    cumulative = cap_d.get("Cumulative capex", [0] * n_years)

    # Compute total construction capex from data
    cons_capex_values = [float(construction_total[i] or 0) for i in range(min(construction_period, n_years))]
    actual_total_construction = sum(cons_capex_values)

    # ── Quarterly capex plan and actuals ──
    # Break annual capex into quarterly S-curve (typical construction spending pattern)
    # Current date: Q1 2026 — we simulate actuals up to now
    current_year = 2026
    current_quarter = 1  # Q1 2026
    ...
    # Quarterly split per year: front-loaded for procurement, back-loaded for installation
    q_splits = [0.15, 0.25, 0.35, 0.25]

    cum_plan = 0.0
    cum_actual = 0.0
    cost_overrun_total = 0.0

    for yr_idx in range(construction_period):
        yr = first_year + yr_idx
        annual_capex = float(construction_total[yr_idx] or 0) if yr_idx < len(construction_total) else 0

        for q in range(4):
            q_label = f"Q{q + 1} {str(yr)[2:]}"
            quarters.append(q_label)
            planned = round(annual_capex * q_splits[q], 1)
            plan_quarterly.append(planned)
            cum_plan += planned
            plan_cumulative.append(round(cum_plan, 1))

            # Simulate actuals: slight over-run on civil works (year 2, Q1–Q2)
            if yr < current_year or (yr == current_year and q + 1 <= current_quarter):
                # Past — generate realistic actuals with slight overrun
                if yr == first_year + 1 and q <= 1:
                    # Civil works overrun in 2026 Q1-Q2
                    overrun_factor = 1.22  # 22% overrun on this phase
                elif yr == first_year and q >= 2:
                    overrun_factor = 1.05  # slight overrun late in year 1
                elif yr == first_year:
                    overrun_factor = 0.98  # slightly under budget early (mobilisation)
                else:
                    overrun_factor = 1.04
                actual = round(planned * overrun_factor, 1)
                cost_overrun_total += actual - planned
            else:
                actual = None  # Future — no actuals yet
            actuals_quarterly.append(actual)
            cum_actual += actual if actual is not None else 0
            actuals_cumulative.append(round(cum_actual, 1) if actual is not None else None)

    # Fill forward cumulative actuals line to show projection
    last_actual_idx = -1
    for i, v in enumerate(actuals_cumulative):
        if v is not None:
            last_actual_idx = i

    # ── Cost overrun KPI ──
    capex_spent = round(cum_actual, 0)
    cost_overrun = round(cost_overrun_total, 0)
    pct_spent = round(cum_actual / total_capex_base * 100, 0) if total_capex_base else 0
    overrun_pct = round(cost_overrun_total / cum_actual * 100, 1) if cum_actual else 0

    # ── Milestones completed and schedule status ──
    milestones = []

    # M1: EIA & regulatory approval (precedes construction)
    milestones.append({
        "id": "M1", "description": "EIA & regulatory approval",
        "plan": "Jun 2025", "actual": "May 2025",
        "variance": "-1 month", "capex": "$0M", "status": "Done"
    })

    # M2: FEED completion
    milestones.append({
        "id": "M2", "description": "FEED completion",
        "plan": "Sep 2025", "actual": "Sep 2025",
        "variance": "On time", "capex": f"${int(round(cons_capex_values[0] * 0.25))}M",
        "status": "Done"
    })

    # M3: EPC contract award
    milestones.append({
        "id": "M3", "description": "EPC contract award",
        "plan": "Nov 2025", "actual": "Oct 2025",
        "variance": "-1 month", "capex": "$0M", "status": "Done"
    })

    # M4: Foundation & civil works (currently in progress — Q1 2026)
    civil_capex = sum(float(tank1[i] or 0) + float(tank2[i] or 0) for i in range(min(2, len(tank1))))
    milestones.append({
        "id": "M4", "description": "Foundation & civil works",
        "plan": "Q1 2026", "actual": "Q1 2026",
        "variance": f"+${int(cost_overrun)}M overrun" if cost_overrun > 0 else "On budget",
        "capex": f"${int(round(civil_capex * 0.55))}M",
        "status": "Watch" if cost_overrun > 5 else "On track"
    })

    # M5: Tank shell fabrication (upcoming)
    tank_fab_capex = sum(float(tank1[i] or 0) + float(tank2[i] or 0) for i in range(min(3, len(tank1)))) * 0.45
    milestones.append({
        "id": "M5", "description": "Tank shell fabrication",
        "plan": "Q3 2026", "actual": "Q3 2026 est",
        "variance": "On track", "capex": f"${int(round(tank_fab_capex))}M",
        "status": "On track"
    })

    # M6: Sub-sea pipeline works
    pipe_capex = sum(float(pipeline[i] or 0) for i in range(min(3, len(pipeline))))
    milestones.append({
        "id": "M6", "description": "Sub-sea pipeline works",
        "plan": "Q4 2026", "actual": "TBD",
        "variance": "At risk", "capex": f"${int(round(pipe_capex))}M",
        "status": "At risk"
    })

    # M7: Tank commissioning
    comm_capex = sum(float(regas[i] or 0) + float(utilities[i] or 0) + float(inst_ctrl[i] or 0)
                     for i in range(min(3, len(regas))))
    milestones.append({
        "id": "M7", "description": "Tank commissioning",
        "plan": f"Q2 {construction_end_year}",
        "actual": "TBD", "variance": "TBD",
        "capex": f"${int(round(comm_capex))}M", "status": "Planned"
    })

    # M8: First gas send-out
    milestones.append({
        "id": "M8", "description": "First gas send-out",
        "plan": f"Jan {ops_start_year}",
        "actual": "TBD", "variance": "TBD",
        "capex": "$0M", "status": "Planned"
    })

    milestones_done = sum(1 for m in milestones if m["status"] == "Done")
    milestones_total = len(milestones)

    # ── IRR reforecast over quarters ──
    # Simulate how IRR changes with cost overrun
    one_way_items = sensitivity_data.get("one_way", [])
    sens_by_label = {item["assumption"]: item for item in one_way_items}

    # Use capex overrun sensitivity to compute IRR impact
    capex_sens = sens_by_label.get("Capex overrun", {})

    irr_reforecast = []
    for i, q_label in enumerate(quarters):
        if actuals_cumulative[i] is not None:
            # Compute overrun % at this point
            if plan_cumulative[i] > 0:
                overrun_at_q = (actuals_cumulative[i] - plan_cumulative[i]) / total_capex_base
            else:
                overrun_at_q = 0
            # Use sensitivity interpolation for capex overrun impact on IRR
            if capex_sens:
                points = [
                    (-0.20, float(capex_sens.get("stress_minus_20", base_irr))),
                    (-0.10, float(capex_sens.get("stress_minus_10", base_irr))),
                    (0.0, float(capex_sens.get("base_case", base_irr))),
                    (0.10, float(capex_sens.get("upside_plus_10", base_irr))),
                    (0.20, float(capex_sens.get("upside_plus_20", base_irr))),
                ]
                x = max(-0.20, min(0.20, overrun_at_q / contingency_pct if contingency_pct else overrun_at_q))
                reforecast_irr = base_irr
                for j in range(len(points) - 1):
                    x0, y0 = points[j]
                    x1, y1 = points[j + 1]
                    if x0 <= x <= x1:
                        t = (x - x0) / (x1 - x0) if x1 != x0 else 0
                        reforecast_irr = y0 + t * (y1 - y0)
                        break
            else:
                # Simple linear: each 1% overrun costs ~0.3pp IRR
                reforecast_irr = base_irr - overrun_at_q * 0.03
            irr_pct = round(reforecast_irr * 100 if reforecast_irr < 1 else reforecast_irr, 2)
        else:
            # Future: project forward based on latest actual
            irr_pct = irr_reforecast[-1]["irr"] if irr_reforecast else round(base_irr * 100, 2)
        irr_reforecast.append({"quarter": q_label, "irr": irr_pct})

    # Extend IRR reforecast into future with flat projection
    last_irr = irr_reforecast[-1]["irr"] if irr_reforecast else round(base_irr * 100, 2)
    for i in range(len(quarters), len(quarters) + 4):
        yr_offset = i // 4
        q_offset = i % 4
        yr = first_year + yr_offset
        q_label = f"Q{q_offset + 1} {str(yr)[2:]}"
        irr_reforecast.append({"quarter": q_label, "irr": last_irr})

    irr_current = irr_reforecast[last_actual_idx]["irr"] if last_actual_idx >= 0 else round(base_irr * 100, 2)
    irr_delta = round(irr_current - base_irr * 100, 1)
    hurdle_rate = 10.0  # Typical hurdle

    # ── Schedule status ──
    has_at_risk = any(m["status"] == "At risk" for m in milestones)
    has_watch = any(m["status"] == "Watch" for m in milestones)
    if has_at_risk:
    ...
    alert_badge = None
    if cost_overrun > 10:
        alert_badge = f"${int(cost_overrun)}M overrun · Watch"
    elif has_at_risk:
        alert_badge = "Schedule at risk"

    result = AnalysisResult(
        scenario_id=scenario_id,
        agent_id="MonitorAgent",
        result_json={"milestones": milestones, "capex_spent": capex_spent},
        confidence=1.0,
    )
    db.add(result)
    db.commit()
    ...
    return {
        "scenario_id": scenario_id,
        "kpis": {
            "capex_spent": int(capex_spent),
            "capex_total": int(total_capex_base),
            "pct_spent": int(pct_spent),
            "cost_overrun": int(cost_overrun),
            "overrun_pct": overrun_pct,
            "overrun_driver": "Civil works",
            "schedule_status": schedule_status,
            "milestones_done": milestones_done,
            "milestones_total": milestones_total,
            "target_completion": f"Q4 {construction_end_year}",
            "irr_reforecast": irr_current,
            "irr_base": round(base_irr * 100, 1),
            "irr_delta_pp": irr_delta,
            "irr_driver": "Driven by overrun" if cost_overrun > 5 else "On plan",
            "hurdle_rate": hurdle_rate,
        },
        "capex_chart": {
            "quarters": quarters,
            "plan": plan_cumulative,
            "actuals": actuals_cumulative,
        },
        "irr_chart": irr_reforecast,
