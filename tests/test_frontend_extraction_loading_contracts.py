...
def test_home_upload_uses_one_request_controller_and_existing_api_once() -> None:
    ...
    assert source.count("uploadModel(file)") == 1
    assert "localStorage.setItem('investiq_model_id', data.model_id)" in source
    assert "localStorage.setItem('investiq_investment_id', data.investment_id)" in source
...
def test_package_keeps_dependencies_and_lint_contract_unchanged() -> None:
    ...
    assert package["scripts"]["test"] == "node --test tests/*.test.cjs"
...
def test_upload_api_surfaces_normalized_backend_detail_without_extra_requests() -> None:
    source = read(API)

    assert "normalizeUploadResponseError" in source
    assert source.count("fetch(`${API_BASE}/api/v1/models/upload`") == 1
    assert "await res.json()" in source
